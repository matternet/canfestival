...
bool_each = lambda fns, args=[]: cf_bools(apply_each(fns, args))
conjoin = lambda fns, args=[]: reduce(mul, cf_bool_each(fns, args))
...
disjoin = lambda fns, args=[]: reduce(add, cf_bool_each(fns, args))
